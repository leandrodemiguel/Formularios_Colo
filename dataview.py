...
def dataview(data, tipoDeFormulario):
    ...
    if (tipoDeFormulario == "firmas"):
        ...
        df['Inicio del curso'] = pd.to_datetime(df['Inicio del curso']).dt.strftime('%Y-%m-%d', errors="coerce")
        df['Fin del curso'] = pd.to_datetime(df['Fin del curso']).dt.strftime('%Y-%m-%d', errors="coerce")
